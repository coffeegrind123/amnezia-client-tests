...
private slots:
    ...
    void init() {
        m_settings->clearSettings();
        if (m_coreController->serversModel()) {
            m_coreController->serversModel()->updateModel(QVector<ServerDescription>(), QString());
        }
    }
    ...
    void testUiServersModelAndControllerRoles() {
        ...
        m_coreController->importCoreController()->importConfig(testConfig);
        
        QVERIFY2(importFinishedSpy.count() == 1, "importFinished signal should be emitted");
        QVERIFY2(m_coreController->serversRepository()->serversCount() == 1, "Server should be imported");
        
        int serverIndex = m_coreController->serversRepository()->defaultServerIndex();
        QVERIFY2(serverIndex == 0, "Default server index should be 0");
        
        if (m_coreController->serversModel()) {
            QVERIFY2(m_coreController->serversModel()->rowCount() == 1, "ServersModel should have 1 row");
            
            QModelIndex serverModelIndex = m_coreController->serversModel()->index(0, 0);
            QVERIFY2(serverModelIndex.isValid(), "Server model index should be valid");
            
            QString serverName = m_coreController->serversModel()->data(serverModelIndex, ServersModel::NameRole).toString();
            ...
            QString serverDescription = m_coreController->serversModel()->data(serverModelIndex, ServersModel::ServerDescriptionRole).toString();
            ...
            QString hostName = m_coreController->serversModel()->data(serverModelIndex, ServersModel::HostNameRole).toString();
            QVERIFY2(hostName == "test.example.com", "Host name should match");
            
            bool isDefault = m_coreController->serversModel()->data(serverModelIndex, ServersModel::IsDefaultRole).toBool();
            QVERIFY2(isDefault == true, "Server should be default");
            
            bool hasInstalledContainers = m_coreController->serversModel()->data(serverModelIndex, ServersModel::HasInstalledContainers).toBool();
            QVERIFY2(hasInstalledContainers == true, "Server should have installed containers");
            
            bool hasWriteAccess = m_coreController->serversModel()->data(serverModelIndex, ServersModel::HasWriteAccessRole).toBool();
            QVERIFY2(hasWriteAccess == false, "Server should not have write access for imported config");
            
            int defaultContainerRole = m_coreController->serversModel()->data(serverModelIndex, ServersModel::DefaultContainerRole).toInt();
        ...
        if (m_coreController->serversUiController()) {
            m_coreController->serversUiController()->setProcessedServerId(
                    m_coreController->serversUiController()->getServerId(0));

            QString hostName = "test.example.com";
            
            QString collapsedDescription = m_coreController->serversUiController()->getDefaultServerDescriptionCollapsed();
            ...
            QString expandedDescription = m_coreController->serversUiController()->getDefaultServerDescriptionExpanded();
        ...
        if (m_coreController->containersModel()) {
            ...
            QModelIndex containerModelIndex = m_coreController->containersModel()->index(awgContainerIndex, 0);
            QVERIFY2(containerModelIndex.isValid(), "Container model index should be valid");
            
            bool isInstalled = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::IsInstalledRole).toBool();
            QVERIFY2(isInstalled == true, "Awg container should be installed");
            
            bool isVpnContainer = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::IsVpnContainerRole).toBool();
            QVERIFY2(isVpnContainer == true, "Awg container should be VPN container");
            
            QString containerName = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::NameRole).toString();
            ...
            QString containerDescription = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::DescriptionRole).toString();
            ...
            QString detailedDescription = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::DetailedDescriptionRole).toString();
            ...
            int serviceType = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::ServiceTypeRole).toInt();
            QVERIFY2(serviceType == static_cast<int>(ProtocolEnumNS::ServiceType::Vpn), "Service type should be Vpn");
            
            bool isSupported = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::IsSupportedRole).toBool();
            QVERIFY2(isSupported == true, "Container should be supported");
            
            bool isShareable = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::IsShareableRole).toBool();
            QVERIFY2(isShareable == true, "Container should be shareable");
            
            QJsonObject containerConfig = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::ConfigRole).toJsonObject();
            ...
            bool isThirdParty = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::IsThirdPartyConfigRole).toBool();
            QVERIFY2(isThirdParty == true, "Imported config should be third party config");
            
            DockerContainer dockerContainer = static_cast<DockerContainer>(m_coreController->containersModel()->data(containerModelIndex, ContainersModel::DockerContainerRole).toInt());
            QVERIFY2(dockerContainer == DockerContainer::Awg, "Docker container should be Awg");
            
            QString containerString = m_coreController->containersModel()->data(containerModelIndex, ContainersModel::ContainerStringRole).toString();
    ...
    void testServerDescriptionFormat() {
        ...
        m_coreController->importCoreController()->importConfig(configNoDns);
        QVERIFY2(importFinishedSpy.count() == 1, "importFinished should be emitted");
        m_coreController->appSettingsRepository()->setUseAmneziaDns(false);
        QVector<ServerDescription> descriptionsNoDns = m_coreController->serversController()->buildServerDescriptions(
            m_coreController->appSettingsRepository()->useAmneziaDns());
        const QString defIdNoDns = m_coreController->serversRepository()->defaultServerId();
        m_coreController->serversModel()->updateModel(descriptionsNoDns, defIdNoDns);

        QString descNoDns = m_coreController->serversModel()->data(
            m_coreController->serversModel()->index(0, 0), ServersModel::ServerDescriptionRole).toString();
        ...
        m_coreController->serversRepository()->clearServers();
        if (m_coreController->serversRepository()->serversCount() > 0) {
            m_coreController->serversRepository()->setDefaultServer(m_coreController->serversRepository()->serverIdAt(0));
        }

        QJsonObject configWithDns = createServerDescriptionTestConfig(true);
        m_coreController->importCoreController()->importConfig(configWithDns);
        QVERIFY2(m_coreController->serversRepository()->serversCount() == 1, "Server should be imported");
        m_coreController->appSettingsRepository()->setUseAmneziaDns(true);
        QVector<ServerDescription> descriptionsWithDns = m_coreController->serversController()->buildServerDescriptions(
            m_coreController->appSettingsRepository()->useAmneziaDns());
        const QString defIdWithDns = m_coreController->serversRepository()->defaultServerId();
        m_coreController->serversModel()->updateModel(descriptionsWithDns, defIdWithDns);

        QString descWithDns = m_coreController->serversModel()->data(
            m_coreController->serversModel()->index(0, 0), ServersModel::ServerDescriptionRole).toString();
