...
private slots:
    ...
    void testServerEditTriggersHandlers() {
        ...
        QVERIFY2(serverEditedSpy.at(0).at(0).toString() == m_coreController->serversRepository()->serverIdAt(0),
                 "serverEdited should emit edited server id");

        const QString editedDesc = amnezia::test::serverDescription(m_coreController->serversRepository(),
    ...
    void testServerEditPreservesDefault() {
        ...
        QVERIFY2(m_coreController->serversRepository()->defaultServerIndex() == 1, "Default server should be index 1");

        QSignalSpy defaultServerChangedSpy(m_coreController->serversRepository(), &SecureServersRepository::defaultServerChanged);

        amnezia::test::setServerDescription(m_coreController->serversRepository(),
                                            m_coreController->serversController()->getServerId(1),
        ...
        QVERIFY2(m_coreController->serversRepository()->defaultServerIndex() == 1, "Default server index should remain 1");
