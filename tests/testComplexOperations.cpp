...
private slots:
    ...
    void testComplexOperationSequence() {
        ...
        QSignalSpy serverAddedSpy(m_coreController->serversRepository(), &SecureServersRepository::serverAdded);
        QSignalSpy serverEditedSpy(m_coreController->serversRepository(), &SecureServersRepository::serverEdited);
        ...
        QString editedDesc0 = amnezia::test::serverDescription(m_coreController->serversRepository(),
                                                               m_coreController->serversRepository()->serverIdAt(0));
        QVERIFY2(editedDesc0 == "Edited First Server", "First server should be edited");

        m_coreController->serversController()->removeServer(m_coreController->serversController()->getServerId(1));
        ...
        QVERIFY2(m_coreController->serversRepository()->defaultServerIndex() == 1, "Default should be index 1 (was 2, removed 1)");
        ...
        QVERIFY2(m_coreController->serversRepository()->defaultServerIndex() == 0, "Default should be index 0");
        ...
        QString finalDesc0 = amnezia::test::serverDescription(m_coreController->serversRepository(),
                                                              m_coreController->serversRepository()->serverIdAt(0));
        QVERIFY2(finalDesc0 == "Final Edited Server", "First server should be edited again");

        QVERIFY2(m_coreController->serversRepository()->serversCount() == 2, "Final servers count should be 2");
        QVERIFY2(m_coreController->serversRepository()->defaultServerIndex() == 0, "Final default index should be 0");

        if (m_coreController->serversModel()) {
            QVERIFY2(m_coreController->serversModel()->rowCount() == 2, "Model should have 2 rows");
            QString modelDesc0 = m_coreController->serversModel()->data(m_coreController->serversModel()->index(0, 0), ServersModel::NameRole).toString();
