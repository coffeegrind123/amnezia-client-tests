...
private:
    ...
    {
        const QString serverId = m_coreController->serversRepository()->serverIdAt(serverIndex);
        const auto adminCfg = m_coreController->serversRepository()->selfHostedAdminConfig(serverId);
        ...
        if (m_coreController->serversModel()) {
            bool hasWriteAccess = m_coreController->serversModel()->data(
                m_coreController->serversModel()->index(serverIndex, 0),
...
private slots:
    ...
    void testSelfHostedServerSetup() {
        ...
        ErrorCode installServerError = m_coreController->installController()->installServer(
        ...
        int serverIndex = m_coreController->serversRepository()->serversCount() - 1;
        qDebug() << "Server with Awg container added at index:" << serverIndex;
        
        const auto adminAfterAwg = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
        ...
        const QString serverIdForOps = m_coreController->serversRepository()->serverIdAt(serverIndex);
        ErrorCode installContainerError = m_coreController->installController()->installContainer(
        ...
        const auto adminAfterDns = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
    ...
    void testSelfHostedServerEmptyRecover() {
        ...
        serverConfig.description = m_coreController->serversRepository()->nextAvailableServerName();
        ...
        QSignalSpy serverAddedSpy(m_coreController->serversRepository(), &SecureServersRepository::serverAdded);
        m_coreController->serversRepository()->addServer(QString(), serverConfig.toJson(),
        ...
        const auto addedAdmin = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
        ...
        const QString scanServerId = m_coreController->serversRepository()->serverIdAt(serverIndex);
        ErrorCode scanError = m_coreController->installController()->scanServerForInstalledContainers(scanServerId);
        ...
        const auto scannedAdmin = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
    ...
    void testRemoveAllContainers() {
        ...
        const auto adminBeforeRemoval = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
        ...
        const QString removeServerId = m_coreController->serversRepository()->serverIdAt(serverIndex);
        ErrorCode removeError = m_coreController->installController()->removeAllContainers(removeServerId);
        ...
        const auto adminAfterRemoval = m_coreController->serversRepository()->selfHostedAdminConfig(
            m_coreController->serversRepository()->serverIdAt(serverIndex));
